...
class SummaryAdapter(private val summaries: List<Summary>): RecyclerView.Adapter<SummaryAdapter.SummaryViewHolder>() {
    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): SummaryViewHolder = SummaryViewHolder(ItemSimpleOneLineBinding.inflate(LayoutInflater.from(parent.context), parent, false))
    ...
    inner class SummaryViewHolder(val binding: ItemSimpleOneLineBinding): RecyclerView.ViewHolder(binding.root) {
        fun bind(summary: Summary) {
            binding.itemSimpleOneLineTextViewTitle.text = summary.title
            binding.itemSimpleOneLineTextViewSubtitle.text = summary.price.toString()
